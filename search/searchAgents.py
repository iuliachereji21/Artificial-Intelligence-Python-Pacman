...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.startState = (self.startingPosition, [0,1,2,3])
        """an array with the indexes of the corners in self.corners that have not been visited"""
    ...
    def getNextState(self, state, action):
        ...
        arr = state[1].copy()
        for i in range(4):
            if nextx == self.corners[i][0] and nexty == self.corners[i][1] and i in arr:
                arr.remove(i)
                """we got to a corner"""
        return ((nextx,nexty), arr)
...
"""maximum=0
    for i in range(len(listOfFood)):
        probl=PositionSearchProblem(gameState=problem.startingGameState,goal=listOfFood[i],start=position,warn=False,visualize=False)
        dist=len(search.bfs(probl))
        if dist>maximum:
            maximum=dist
    return maximum"""
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        if self.food[x][y]==True: return True
        else: return False
